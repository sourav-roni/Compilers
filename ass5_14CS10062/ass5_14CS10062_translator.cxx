...
void convert2bool(expr *e) 
{
	...
	else
	{
		...
		e->falselist = makelist(next_instr+1);
		char * temp1 = convert2char_star("0");
		char * temp2 = strdup((e->location)->name);
		row_of_quad truequad(temp2,temp1,0,equal_to_gop);
		add(truequad);
...
row_of_symbol_table::row_of_symbol_table(char *name1,type_expression *type1,int size1,int offset1,void *initial_value1,symbol_table *nested,char* name2)
{ 
    name = name1;
    type = type1;
    size = size1;
...
int main()
{
  ...
  current = global;
  node = (vault *)malloc(1000*sizeof(vault));						
